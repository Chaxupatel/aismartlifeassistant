...
enum ReminderTab { active, completed }
...
class _ReminderListScreenState extends ConsumerState<ReminderListScreen> {
  ...
  Timer? _refreshTimer;
  ReminderTab _selectedTab = ReminderTab.active;
  ...
  Widget _buildTabSelector(bool isDark, int activeCount, int completedCount) {
    return Padding(
          ...
          children: [
            Expanded(
              child: _buildTabButton(
                ...
                count: activeCount,
                isSelected: _selectedTab == ReminderTab.active,
                onTap: () => setState(() => _selectedTab = ReminderTab.active),
                isDark: isDark,
              ),
            ),
            ...
            Expanded(
              child: _buildTabButton(
                ...
                isSelected: _selectedTab == ReminderTab.completed,
                onTap: () => setState(() => _selectedTab = ReminderTab.completed),
                isDark: isDark,
              ),
            ),
          ],
        ...
        child: Row(
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: isSelected ? Colors.white.withValues(alpha: 0.2) : (isDark ? Colors.white12 : Colors.black12),
  ...
  Widget _buildReminderCard(Reminder reminder, bool isDark) {
  ...
  Widget build(BuildContext context) {
    ...
    // Partition reminders into active and completed
    final activeReminders = filteredReminders.where((r) => !r.isCompleted).toList();
    final completedReminders = filteredReminders.where((r) => r.isCompleted).toList();

    final currentList = _selectedTab == ReminderTab.active ? activeReminders : completedReminders;
        ...
        children: [
          Column(
            ...
            children: [
              ...
              // Tabs Row (Always visible side-by-side)
              _buildTabSelector(isDark, activeReminders.length, completedReminders.length),
              const SizedBox(height: AppSizes.m),
              ...
              Expanded(
                child: Padding(
                  ...
                  child: currentList.isEmpty
                      ? _buildEmptyState(
                          _selectedTab == ReminderTab.active ? 'No active reminders.' : 'No completed reminders.',
                          isDark,
                        )
                      : ListView.builder(
                          padding: const EdgeInsets.only(
                            left: AppSizes.xs,
                            right: AppSizes.xs,
                            bottom: 240, // Avoid bottom navigation and ad overlap
                          ),
                          itemCount: currentList.length + (currentList.length / _adInterval).floor(),
                          itemBuilder: (context, index) {
                            if (_isAdIndex(index)) {
                              return Padding(
                                padding: const EdgeInsets.only(bottom: AppSizes.m),
                                child: GlassNativeAdWidget.small(),
                              );
                            }
                            final reminderIndex = _getReminderIndex(index);
                            final reminder = currentList[reminderIndex];
